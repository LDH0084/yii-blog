...
class TechnologyComment extends ActiveRecord
{
    ...
    //获取用户列表
    public static function getTechnologyCommentList($post)
    {
        $page = $post['page'];
        $rows = $post['rows'];
        $sort = $post['sort'];
    ...
    //删除一条或多条技术博文
    public static function deleteTechnologyComment($post)
    {
        $sql = "DELETE FROM {{%technology_comment}} WHERE id IN ($post)";
